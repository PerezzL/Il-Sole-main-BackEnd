...
exports.getAllExpendios = async (req, res) => {
  try {
    const result = await client.query('SELECT * FROM Expendio');
...
exports.getExpendioById = async (req, res) => {
  const { id } = req.params;
  try {
    const result = await client.query('SELECT * FROM Expendio WHERE id = $1', [id]);
...
exports.deleteExpendio = async (req, res) => {
  const { id } = req.params;
  try {
    const result = await client.query('DELETE FROM Expendio WHERE id = $1 RETURNING *', [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Expendio no encontrado' });
    }
